...
export default class DatePicker extends React.Component {
  ...
  handleChange = (...allArgs) => {
    ...
    let date = new Date();
    let inputDate = event.target.value.split("/");

    if (!!inputDate[0] && inputDate[0].trim() !== "" && !isNaN(inputDate[0])) {
      date.setDate(inputDate[0]);
    }

    if (!!inputDate[1] && inputDate[1].trim() !== "" && !isNaN(inputDate[1])) {
      date.setMonth(inputDate[1] - 1);
    }

    if (!!inputDate[2] && inputDate[2].trim() !== "" && !isNaN(inputDate[2])) {
      if (inputDate[2].length <= 2) {
        date = parseDate(event.target.value, "dd/MM/yy", this.props.locale);
      } else {
        date = parseDate(event.target.value, "dd/MM/yyyy", this.props.locale);
      }
    }
